...
const QrCodeList = (url) => {
  ...
  const [searchParam] = useState(["nombre_ref", "data", "created_at"]);

  useEffect(() => {
    const fetchQrCodes = async () => {
      try {
        const response = await fetch('http://localhost/gatsby-qr/v1/list-qr.php');
        if (!response.ok) {
          ...
          {search(qrCodes).map((qrCode) => (
            ...
            sx={{
              ...
              <CardContent>
                ...
                <Typography variant="h6">{qrCode.nombre_ref}</Typography>
                <Typography variant="body2">Data: {qrCode.data}</Typography>
                <Typography variant="body2">Descripción: {qrCode.descripcion}</Typography>
                <Typography variant="body2">Fecha Creación: {qrCode.created_at}</Typography>
